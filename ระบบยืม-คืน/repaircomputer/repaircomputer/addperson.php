...
<h1>ระบบซ่อมคอมพิวเตอร์ <?php echo $psid == '' ? 'Add' : ' '; ?> เพิ่มข้อมูลลูกค้า</h1>
     <form method="post">
        <div class="mb-3">
            <label for="psid" class="form-label">รหัสผู้แจ้งซ่อม</label>
            <input name="psid" type="text" class="form-control" id="psid" value="<?php echo empty($psid) ? '' : $rowitem["psid"]; ?>" <?php echo empty($psid) ? '' : 'disabled'; ?>>
        </div>
        <div class="mb-3">
            <label for="psname" class="form-label">ชื่อผู้แจ้งซ่อม</label>
            <input name="psname" type="text" class="form-control" id="psname" value="<?php echo empty($psid) ? '' : $rowitem["psname"]; ?>">
        </div>
        <div class="mb-3">
            <label for="psphone" class="form-label">เบอร์โทรศัพท์ผู้แจ้งซ่อม</label>
            <input name="psphone" type="text" class="form-control" id="psphone" value="<?php echo empty($psid) ? '' : $rowitem["psphone"]; ?>">
        </div>

        <div class="mb-3">
            <label for="psdate" class="form-label">วันเข้าทำงานผู้แจ้งซ่อม</label>
            <input name="psdate" type="text" class="form-control" id="psdate" value="<?php echo empty($psid) ? '' : $rowitem["psdate"]; ?>">
        </div>
