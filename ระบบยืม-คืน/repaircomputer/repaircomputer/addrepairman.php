...
<h1>ระบบซ่อมคอมพิวเตอร์ <?php echo $mid == '' ? 'Add' : ' '; ?> เพิ่มข้อมูลผู้แจ้งซ่อม</h1>
     <form method="post">
        <div class="mb-3">
            <label for="mid" class="form-label">รหัสผู้แจ้งซ่อม</label>
            <input name="mid" type="text" class="form-control" id="mid" value="<?php echo empty($mid) ? '' : $rowitem["mid"]; ?>" <?php echo empty($mid) ? '' : 'disabled'; ?>>
        </div>
        <div class="mb-3">
            <label for="mname" class="form-label">ชื่อผู้แจ้งซ่อม</label>
            <input name="mname" type="text" class="form-control" id="mname" value="<?php echo empty($mid) ? '' : $rowitem["mname"]; ?>">
        </div>
        <div class="mb-3">
            <label for="mphone" class="form-label">เบอร์โทรศัพท์ผู้แจ้งซ่อม</label>
            <input name="mphone" type="text" class="form-control" id="mphone" value="<?php echo empty($mid) ? '' : $rowitem["mphone"]; ?>">
        </div>

        <div class="mb-3">
            <label for="mdate" class="form-label">วันเข้าทำงานผู้แจ้งซ่อม</label>
            <input name="mdate" type="text" class="form-control" id="mdate" value="<?php echo empty($mid) ? '' : $rowitem["mdate"]; ?>">
        </div>
