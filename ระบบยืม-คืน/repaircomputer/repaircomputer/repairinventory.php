...
<body class = "container-fluid">
    <h1>อุปกรณ์ซ่อมคอมพิวเตอร์</h1>
    <a class="btn btn-primary float-end bi-cart-plus" href="addrepairinventory.php" role="button"> อุปกรณ์ </a>
        <form class="row row-cols-lg-auto g-3 align-items-center" method="get" action="">
            ...
            <div class="col-3">
                ...
                <select name="repairinventorys" class="form-select" id="repairinventorys">
                    <option value="">รหัสหมวดหมู่</option>
                    <?php 
                        $resulttype = $conn->query("select * from repairinventory");
                        while($row = $resulttype->fetch_assoc()){
                        echo "<option value=\"".$row["ctgrid"]."\"";
                        if($row["ctgrid"]==$repairinventorys){
                            echo "selected";
                        }
    ...
    <p>อุปกรณ์ซ่อมคอมพิวเตอร์ทั้งหมด <?php echo $no;?> รายการ.</p>
