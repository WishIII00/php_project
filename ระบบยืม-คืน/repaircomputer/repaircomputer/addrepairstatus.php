...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    if(empty($ssid)){
        ...
        $statement->bind_param('issi', $ssid, $sstatus, $ssdetail, $ssdate); 
        ...
        $statement = $conn->prepare($sql); 
        $statement->bind_param('ssii',  $sstatus, $ssdetail, $ssdate, $ssid,); 
        $result = $statement->execute(); 
        if (!$result) { 
            die('Execute failed: ' . $statement->error); 
        }
...
<h1>ระบบซ่อมคอมพิวเตอร์ <?php echo $ssid == '' ? 'Add' : ''; ?> เพิ่มข้อมูลสถานะแจ้งซ่อม</h1>
     <form method="post">
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="ssdetail" class="form-label">รายละเอียด</label>
            <input name="ssdetail" type="text" class="form-control" id="ssdetail" value="<?php echo empty($ssid) ? '' : $rowitem["ssdetail"]; ?>">
        </div>
        <div class="mb-3">
            <label for="ssdate" class="form-label">วันที่ลูกค้าเอาของมาส่งซ่อม</label>
            <input name="ssdate" type="text" class="form-control" id="ssdate" value="<?php echo empty($ssid) ? '' : $rowitem["ssdate"]; ?>">
        </div>
